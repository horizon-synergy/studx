...
export default function Navbar() {
  ...
  const { notifications, unreadChats, unreadNotifs, markRead, markAllRead } =
    useNotifications();
    ...
    <header className={s.header} ref={navRef}>
      <div className={s.inner}>
        <NavLink to="/" className={s.brand}>
          <div className={s.logoMark}>
            <span className={s.logoText}>SX</span>
          </div>
          ...
          {currentUser && (
            <>
              <NavLink to="/messages" className={cls}>
                Messages
                {unreadChats > 0 && (
                  <span className={s.badge}>{unreadChats}</span>
                )}
              </NavLink>
        ...
        <div className={s.controls}>
          ...
          {currentUser ? (
            <>
              ...
              <div className={s.notifWrap} ref={notifRef}>
                ...
                >
                  ...
                  {totalUnread > 0 && (
                    <span className={`${s.badge} ${s.badgePulse}`}>
                      {totalUnread}
                    </span>
                  )}
                </button>
              ...
              <button
                className={`${s.iconBtn} ${s.logoutBtn}`}
            ...
            <>
              ...
              <NavLink to="/register" className={s.ctaBtn}>
                Sign Up
              </NavLink>
            </>
          )}
      ...
      {open && (
        <nav className={s.mobileNav}>
          <NavLink to="/" className={cls} end onClick={() => setOpen(false)}>
            Marketplace
          </NavLink>
          <NavLink to="/eats" className={cls} onClick={() => setOpen(false)}>
            StudX Eats
          </NavLink>
          {currentUser ? (
            <>
